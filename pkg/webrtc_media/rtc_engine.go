...
func (s *WebRTCEngine) CreateSenderReciverClient(offer webrtc.SessionDescription, pc **webrtc.PeerConnection, addVideoTrack **webrtc.TrackLocalStaticSample, addAudioTrack **webrtc.TrackLocalStaticRTP, stop chan int, pli chan int) (answer webrtc.SessionDescription, err error) {
	*pc, err = s.api.NewPeerConnection(s.cfg)
	...
	(*pc).OnTrack(func(t *webrtc.TrackRemote, receiver *webrtc.RTPReceiver) {
		...
		if t.Kind() == webrtc.RTPCodecTypeVideo {
			...
			s.handleIncomingTrackWithPLI(t, *pc, stop, pli, localVideoTrack, nil)
			...
			s.handleIncomingTrackWithPLI(t, *pc, stop, pli, nil, localAudioTrack)
		}
	})
...
func (s *WebRTCEngine) handleIncomingTrackWithPLI(t *webrtc.TrackRemote, pc *webrtc.PeerConnection, stop, pli chan int, videoTrack *webrtc.TrackLocalStaticSample, audioTrack *webrtc.TrackLocalStaticRTP) {
	if t.Codec().MimeType == webrtc.MimeTypeVP8 ||
		...
		builder := samplebuilder.New(34, pkt, t.Codec().ClockRate)
		for {
			select {
			case <-stop:
				return
